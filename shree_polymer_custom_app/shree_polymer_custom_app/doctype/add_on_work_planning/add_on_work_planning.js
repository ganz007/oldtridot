...
frappe.ui.form.on('Add On Work Planning', {
	refresh: function (frm) {
		...
		frappe.call({
			method: 'shree_polymer_custom_app.shree_polymer_custom_app.doctype.add_on_work_planning.add_on_work_planning.get_work_mould_filters',
			args: {
			},
			freeze: true,
			callback: function (r) {
	...
	"append_jc_and_wo": (frm) => {
		if (!frm.doc.job_card_wo) {
			frm.add_custom_button(__('Create Work Order/Job Card'), function () {
				frappe.call({
					method: 'shree_polymer_custom_app.shree_polymer_custom_app.doctype.add_on_work_planning.add_on_work_planning.submit_workplan',
					args: {
