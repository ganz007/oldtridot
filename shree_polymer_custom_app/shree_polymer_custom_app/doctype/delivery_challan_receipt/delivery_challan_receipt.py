	...
	try:
		...
		if cint(is_internal_mixing)==0: 
			check_item_qty = frappe.db.sql(""" SELECT SD.item_code,SB.qty,DC.name as dc_no,DC.operation,
											...
											AND (DC.received_status<> 'Completed' OR DC.received_status IS NULL)  ORDER BY SD.creation DESC limit 1""",{"warehouse":warehouse,"bar_code":batch_no},as_dict=1)
		...
		for dc_item in dc_items:
			...
			if m_item:
				for x in m_item:
					...
					frappe.db.set_value("Delivery Note Item",x.name,"dc_receipt_date",getdate())
					frappe.db.commit()
		return {"status":"Success","st_entry":stock_entry}
	except Exception as e:
		frappe.log_error(message=frappe.get_traceback(),title="DC Receipt Stock Entry Error")
...
def make_stock_entry_final_batch(mt_doc,item_code,dc_no,spp_settings,work_order_id,compound_code, purpose, qty=None):
	try:
		...
		# for x in items:
		for x in st_items:
			m_item = frappe.db.get_all("Delivery Note Item",filters={"parent":dc_no,"parenttype":"Delivery Note","item_code":x.item_code,"scan_barcode":x.scan_barcode})
			if m_item:
				for x in m_item:
					frappe.db.set_value("Delivery Note Item",x.name,"is_received",1)
					frappe.db.set_value("Delivery Note Item",x.name,"dc_receipt_no",mt_doc.name)
