	                ...
	                callback: function(r) {
	                	if(!r.message.status){
	                		...
	                		if (check_cut_items==1){
		                		...
		                		row.item_produced = st_details[0].item_produced;
		                		if(frm.doc.material_transfer_type=="Transfer Compound to Sheeting Warehouse"){
	...
	refresh: function(frm) {
		...
		if(frm.doc.dc_no){
			frm.add_custom_button(__("View DC"), () => {
				frappe.set_route("Form", "Delivery Note", frm.doc.dc_no);
	    ...
	    if (has_common(frappe.user_roles, ['Batch Operator']) && frappe.session.user!="Administrator") {
	   	 set_field_options("material_transfer_type", ["Transfer Batches to Mixing Center"])
	    }
