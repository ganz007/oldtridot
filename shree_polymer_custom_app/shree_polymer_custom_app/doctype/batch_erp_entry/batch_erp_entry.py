...
from shree_polymer_custom_app.shree_polymer_custom_app.doctype.batch_erp_entry.importer import Importer
...
class BatchERPEntry(Document):
	...
	def get_importer(self):
		return Importer("Imported Batches", data_import=self)


@frappe.whitelist()
def get_preview_from_template(data_import, import_file=None, google_sheets_url=None):
	return frappe.get_doc("Batch ERP Entry", data_import).get_preview_from_template(
...
def form_start_import(data_import):
	return frappe.get_doc("Batch ERP Entry", data_import).start_import()


def start_import(data_import):
	"""This method runs in background job"""
	data_import = frappe.get_doc("Batch ERP Entry", data_import)
	# try:
	i = Importer("Imported Batches", data_import=data_import)
	i.import_data()
	# data_import.db_set("status", "Success")
	# except Exception:
	# 	frappe.db.rollback()
	# 	data_import.db_set("status", "Error")
	# 	# data_import.log_error("Batch ERP Entry failed")
	# 	frappe.log_error(frappe.get_traceback(),"Data import failed")
	# finally:
...
def download_errored_template(data_import_name):
	data_import = frappe.get_doc("Batch ERP Entry", data_import_name)
...
def get_import_status(data_import_name):
	...
	total_payload_count = frappe.db.get_value("Batch ERP Entry", data_import_name, "payload_count")
...
def import_file(doctype, file_path, import_type, submit_after_import=False, console=False):
	"""
	Import documents in from CSV or XLSX using Batch ERP Entry.
	...
	data_import = frappe.new_doc("Batch ERP Entry")
