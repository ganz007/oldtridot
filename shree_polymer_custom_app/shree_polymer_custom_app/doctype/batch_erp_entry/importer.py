...
class Importer:
	...
	def insert_record(self, doc):
		...
		new_doc.data_import  = self.data_import.name
		new_doc.insert()
...
class ImportFile:
	...
	def parse_data_from_template(self):
		...
		for i, row in enumerate(self.raw_data):
			...
			if not header:
				...
				if not frappe.db.get_all("Imported Batches",filters={"compound_code":row[0],"mixbatchno":row[1],"mix_barcode":row[2],"material":row[4]}):
					...
					data.append(row_obj)
					p_doc = row_obj.parse_doc(doctype="Imported Batches")
					parsed_data.append(p_doc)
					material_list.append(p_doc.get("material"))
		self.header = header
		self.columns = self.header.columns
		self.data = data
		# frappe.log_error(data,"imported")
		...
		if len(data) < 1:
			...
			before_validate_tqty(parsed_data,material_list)
...
def before_validate_tqty(rows,material_list):
	m_list = []
	...
	spp_settings = frappe.get_single("SPP Settings")
	for x in m_list:
		qty = sum(flt(m.get('actual_weight'),3) for m in rows if m.get("material")==x)
		...
		t_qty_list = frappe.db.sql(""" SELECT SUM(qty) as total_qty FROM `tabItem Batch Stock Balance` SB 
										...
										,{"item_code":x,"warehouse":spp_settings.source_warehouse},as_dict=1)
		if t_qty_list:
			if t_qty_list[0].total_qty:
				if not t_qty_list[0].total_qty>=qty:
					frappe.db.rollback()
					frappe.throw("The total available quantity of <b>"+x+"</b> in warehouse <b>"+spp_settings.source_warehouse+"</b> is <b>"+str(t_qty_list[0].total_qty)+"</b>,but total required qty is <b>"+str(qty)+".</b>")
					return False
			else:
				frappe.db.rollback()
				frappe.throw("The total available quantity of <b>"+x+"</b> in warehouse <b>"+spp_settings.source_warehouse+"</b> is <b>0</b>,but total required qty is <b>"+str(qty)+".</b>")
				return False
		else:
			frappe.db.rollback()
			frappe.throw("The total available quantity of <b>"+x+"</b> in warehouse <b>"+spp_settings.source_warehouse+"</b> is <b>0</b>,but total required qty is <b>"+str(qty)+".</b>")
...
def create_wos(batch_entry_id):
	...
	if logs and valid_stock:
		items = frappe.db.sql(""" SELECT  IB.compound_code,IB.mix_barcode FROM `tabImported Batch Log` L INNER JOIN  `tabImported Batches` IB ON L.docname = IB.name 
								  WHERE L.data_import=%(b_id)s AND success=1 group by IB.compound_code,IB.mix_barcode""",{"b_id":batch_entry_id},as_dict=1
										)
			...
			try:
				...
				for item in items:
					if frappe.db.get_all("Item",filters={"name":item.get("compound_code")}):
						...
						if bom:
							# frappe.log_error(item.compound_code,"compound_code")
							# frappe.log_error(item.mix_barcode,"mix_barcode")
							bom_items = list(filter(lambda x: (x.compound_code == item.compound_code and x.mix_barcode == item.mix_barcode), imported_batches))
							actual_weight = sum(flt(e_item.actual_weight) for e_item in imported_batches if e_item.compound_code == item.compound_code and e_item.mix_barcode == item.mix_barcode )
							import time
							...
							r_items = frappe.db.sql(""" SELECT  material,actual_weight FROM  `tabImported Batches` IB 
								  WHERE IB.data_import=%(b_id)s AND IB.compound_code =%(compound_code)s AND IB.mix_barcode=%(mix_barcode)s""",{"b_id":batch_entry_id,'mix_barcode':item.mix_barcode,'compound_code':item.compound_code},as_dict=1
										)
							for r_item in  r_items:
								wo.append("required_items",{
									"item_code":r_item.material,
									"required_qty":r_item.actual_weight,
									"consumed_qty":r_item.actual_weight,
									})
							...
							se = make_stock_entry(batch_entry_id,spp_settings,wo.name,item.get("compound_code"),"Manufacture",mix_barcode = bom_items[0].mix_barcode,mix_batchno = bom_items[0].mixbatchno)
...
def get_aggr_date(compound,mixbatchno):
	...
	frappe.log_error(message=full_date,title='full_date')
	return full_date
...
def update_job_cards(wo,actual_weight,employee,spp_settings):
	...
	for job_card in job_cards:
		...
		# frappe.log_error(message=jc.time_logs,title="jc.time_logs")
		jc_doc.docstatus = 1
		jc_doc.save(ignore_permissions=True)

		

def make_stock_entry(batch_entry_id,spp_settings,work_order_id,compound_code, purpose,mix_barcode,mix_batchno, qty=None):
	...
	try:
		...
		for x in work_order.required_items:
			...
			if t_qty_list:
				if t_qty_list[0].total_qty:
					if t_qty_list[0].total_qty>=x.consumed_qty:
		...
		s_nos = mix_batchno.split(work_order.production_item)
		spp_bno = s_nos[1].replace("_22","").replace("_23","").replace("_24","")
		stock_entry.append("items",{
...
def rollback_transaction(batch_entry_id):
	frappe.db.rollback()
	frappe.db.sql(""" DELETE FROM `tabImported Batches` where data_import =%(batch_entry_id)s """,{"batch_entry_id":batch_entry_id})
	frappe.db.sql(""" DELETE FROM `tabImported Batch Log` WHERE data_import =%(batch_entry_id)s """,{"batch_entry_id":batch_entry_id})
	frappe.db.commit()
	data_import = frappe.get_doc("Batch ERP Entry", batch_entry_id)
